...
export class ColumnLayout extends Layout {
  ...
  public override build(parent: LayoutEntity): void {
    ...
    this.host.getChildLayoutEntities().reduce((y: number, e: LayoutEntity) => {
  ...
  public override calculateContentSize(): Size {
    ...
    const contentSize = this.host
      .getChildLayoutEntities()
      .reduce((size: Size, entity) => {
